...
def forex(
    ...
    return builds(
        Forex, baseCurrency=baseCurrency, quoteCurrency=quoteCurrency, exchange=exchange
    )
...
def instruments(
    ...
    return one_of(
        ...
        currency.flatmap(
            lambda cur: forex(
                baseCurrency=from_type(Currency).filter(lambda cur2: cur2 != cur),
...
def positions(
    ...
    return builds(
        Position, instrument=instrument, quantity=quantity, costBasis=costBasis
    )


def dividendPayments(
    date: SearchStrategy[datetime] = datetimes(),
...
def trades(
    ...
    fees: SearchStrategy[Cash] = cash(quantity=cashAmounts(min_value=Decimal("0"))),
...
def activity(date: SearchStrategy[datetime] = datetimes()) -> SearchStrategy[Activity]:
    return one_of(dividendPayments(date=date), trades(date=date))


def quotes(
    bid: SearchStrategy[Optional[Cash]] = optionals(cash()),
    ...
    return bid.flatmap(
        lambda x: builds(
            Quote,
            bid=just(x),
            ask=ask.map(lambda y: x + abs(y) if x and y else y) if grow_ask else ask,
            last=last,
            close=close,
        )
    )
...
def uniformCurrencyQuotes(
    ...
    return currency.flatmap(
        lambda cur: quotes(
            ...
            close=close.map(lambda x: Cash(currency=cur, quantity=x) if x else None),
            grow_ask=grow_ask,
        )
    )
...
def accountBalances(
    ...
    return builds(
        AccountBalance,
        cash=sets(currencies).flatmap(
            lambda keys: lists(quantities, min_size=len(keys), max_size=len(keys)).map(
                lambda values: {
...
register_type_strategy(
    ...
    from_type(Instrument).flatmap(
        lambda i: trades(
            ...
            fees=cash(
                currency=just(i.currency), quantity=cashAmounts(min_value=Decimal("0"))
            ),
        )
    ),
)
